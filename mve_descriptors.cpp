...
namespace mve {
    ...
    MveDescriptorSetLayout::MveDescriptorSetLayout(MveDevice& mveDevice, std::unordered_map<uint32_t, VkDescriptorSetLayoutBinding> bindings) : mveDevice{ mveDevice }, bindings{ bindings } {
        ...
        if (vkCreateDescriptorSetLayout(mveDevice.device(), &descriptorSetLayoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS) {
            throw std::runtime_error("failed to create descriptor set layout!");
        }
    }

    MveDescriptorSetLayout::~MveDescriptorSetLayout() { //clean up descriptor set layout
        vkDestroyDescriptorSetLayout(mveDevice.device(), descriptorSetLayout, nullptr);
    }

    // *************** Descriptor Pool Builder *********************
    MveDescriptorPool::Builder& MveDescriptorPool::Builder::addPoolSize(VkDescriptorType descriptorType, uint32_t count) {
        poolSizes.push_back({ descriptorType, count }); //implicit construction of VkDescriptorPoolSize struct
        return *this;
    }

    MveDescriptorPool::Builder& MveDescriptorPool::Builder::setPoolFlags(VkDescriptorPoolCreateFlags flags) {
        poolFlags = flags;
        return *this;
    }
    ...
    // *************** Descriptor Pool ********************* 
    MveDescriptorPool::MveDescriptorPool(MveDevice& mveDevice, uint32_t maxSets, VkDescriptorPoolCreateFlags poolFlags, const std::vector<VkDescriptorPoolSize>& poolSizes) : mveDevice{ mveDevice } {
        ...
        if (vkCreateDescriptorPool(mveDevice.device(), &descriptorPoolInfo, nullptr, &descriptorPool) != VK_SUCCESS) {
    ...
    bool MveDescriptorPool::allocateDescriptor(const VkDescriptorSetLayout descriptorSetLayout, VkDescriptorSet& descriptor) const {
    ...
    void MveDescriptorPool::freeDescriptors(std::vector<VkDescriptorSet>& descriptors) const {
        vkFreeDescriptorSets(mveDevice.device(), descriptorPool, static_cast<uint32_t>(descriptors.size()), descriptors.data());
    }
    ...
    // *************** Descriptor Writer *********************
    mveDescriptorWriter::mveDescriptorWriter(MveDescriptorSetLayout& setLayout, MveDescriptorPool& pool) : setLayout{ setLayout }, pool{ pool } {}

    mveDescriptorWriter& mveDescriptorWriter::writeBuffer(uint32_t binding, VkDescriptorBufferInfo* bufferInfo) {
    ...
    mveDescriptorWriter& mveDescriptorWriter::writeImage(uint32_t binding, VkDescriptorImageInfo* imageInfo) {
        assert(setLayout.bindings.count(binding) == 1 && "Layout does not contain specified binding");
